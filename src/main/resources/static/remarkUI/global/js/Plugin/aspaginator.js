      ...
      value: function getDefaults() {
        return {
          ...
          components: {
            prev: {
              tpl: function tpl() {
                return '<li class="' + this.namespace + '-prev page-item"><a class="page-link" v-on:click="pageSeach" aria-label="Prev"><span class="icon wb-chevron-left-mini"></span></a></li>';
              }
            },
            next: {
              tpl: function tpl() {
                return '<li class="' + this.namespace + '-next page-item"><a class="page-link" v-on:click="pageSeach" aria-label="Next"><span class="icon wb-chevron-right-mini"></span></a></li>';
              }
            },
            lists: {
              tpl: function tpl() {
                ...
                for (var k = 1; k < remainder; k++) {
                  lists += '<li class="' + this.namespace + '-items page-item" data-value="' + (this.currentPage - remainder + k) + '"><a class="page-link" v-on:click="pageSeach">' + (this.currentPage - remainder + k) + '</a></li>';
                }
                lists += '<li class="' + this.namespace + '-items page-item ' + this.classes.active + '" data-value="' + this.currentPage + '"><a class="page-link" v-on:click="pageSeach">' + this.currentPage + '</a></li>';
                for (var i = this.currentPage + 1, limit = i + this.visible - remainder - 1 > this.totalPages ? this.totalPages : i + this.visible - remainder - 1; i <= limit; i++) {
                  lists += '<li class="' + this.namespace + '-items page-item" data-value="' + i + '"><a class="page-link" v-on:click="pageSeach">' + i + '</a></li>';
                }
